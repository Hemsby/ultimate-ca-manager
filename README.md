...
![Tests](https://img.shields.io/badge/tests-2949%20passing-brightgreen)
...
- **Certificate Lifecycle** -- Issue, sign, revoke, renew, export (PEM, DER, PKCS#12, JKS), bulk operations, filter by status / issuer / source (ACME, SCEP, EST, AD CS, import…)
...
- [x] **Security hardening & multi-endpoint SCEP** — an audit-driven hardening pass tightens issuance (per-path key-strength floor, CSR EKU capping, gated sub-CA minting), ACME (SAN types bound to validated identifiers, SSRF guard on IP orders), and authorization (mTLS, API-key scoping, TSA, CSRF, SSH, OCSP); named SCEP profiles serve multiple enrollment endpoints, each with its own CA, template, challenge and approval policy; delegated OCSP responder certificates renew automatically *(v2.204)*
