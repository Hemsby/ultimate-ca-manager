...
- **mTLS CA targeting and SSH extension policy follow-ups to the authorization hardening** — issuing an mTLS client certificate with an explicit `ca_id` could still direct *any* CA — the root included — holding only `write:user_certificates`, a self-service scope every operator has and that groups may grant to any user; overriding the configured mTLS CA now additionally requires `write:cas`; passing the configured CA's own id explicitly stays allowed, and the admin/operator flows are unchanged. The account page now mirrors these gates instead of offering actions that can only 403: the mTLS Generate tab appears only with `write:user_certificates` (importing or assigning an existing certificate stays available to everyone), and the issuing-CA dropdown only with `write:cas` — without it the certificate is issued from the configured mTLS CA. Separately, an SSH CA whose default-extensions policy an admin explicitly cleared to "none" was stored as "never configured" and silently reverted to allowing the full standard set — permit-port-forwarding and permit-agent-forwarding included — so requesters could still self-grant the very extensions the policy was meant to forbid; an explicitly empty policy now permits nothing, and CAs that never configured a policy keep serving the standard OpenSSH set unchanged. Contributed by @heidrickla (#253).
...
- **Category-wildcard API keys keep working after an owner demotion** — the re-binding of key scopes to the owner's current permissions dropped `read:*`/`write:*` scopes entirely once the owner no longer held the admin wildcard, because named roles carry only concrete scopes: a demoted admin's key minted from the UI's "read" or "read/write" presets authenticated with **no** permissions and got 403 on every request, even for scopes the owner legitimately retains. Category wildcards now expand to the owner's current scopes in that category, so a key is worth at most — and now also at least — what its owner currently holds. The earlier upgrade note stands for scopes the owner genuinely lost; keys no longer need re-minting for scopes the owner still has. Contributed by @heidrickla (#253).
...
- **The TSA dedicated-certificate requirement is configurable** — the `tsa_require_dedicated_cert` switch added in #246 was settable only by editing its database row (it appeared in neither `GET`/`PATCH /api/v2/tsa/config` nor the UI); it is now exposed in both and as a toggle on the TSA settings page. The default is unchanged (off), so existing deployments keep signing with the CA certificate until they opt in. Contributed by @heidrickla (#253).
