...
- **The encryption require-flags now refuse the way they promise** — follow-up to #245: `UCM_REQUIRE_DB_ENCRYPTION_KEY` was only checked on the first decryption attempt — which never happens at startup, and which swallowed the refusal and handed every integration secret back as "no value", so a deployment that set the flag without a key booted normally and its DNS, LDAP, SMTP, SSO and webhook integrations simply stopped working with nothing pointing at the cause. The flag now refuses at startup — including when the key is present but not a parseable Fernet key (e.g. the output of `openssl rand -hex 32`), which would otherwise boot and fail only at first cipher use — and a configuration refusal during decryption is a hard error instead of a silent empty result. `UCM_REQUIRE_KEY_ENCRYPTION` could be bypassed by a malformed or truncated key: encryption quietly switched off and private keys were written to the database in plaintext — exactly the outcome the flag exists to refuse; an invalid key is now as fatal as a missing one. The disable-encryption endpoint refuses up front while the flag is set, instead of decrypting the database first and leaving a service that cannot boot. Both flags also accept `1`/`yes`/`on` like every other UCM toggle. Deployments that set neither flag keep booting, and keep reading and writing their existing data, unchanged. **Upgrade note:** one change is not flag-gated — `decrypt_value()` now surfaces a cipher-construction failure instead of returning `None`, so a deployment whose `UCM_DB_ENCRYPTION_KEY` is set to a value Fernet cannot parse gets a loud error where it previously read every secret back as "no value" (writes already failed the same way); deployments with no key set, or with a valid one, are unaffected. Contributed by @heidrickla (#252).
