...
- **CSR-supplied Extended Key Usages are capped to the certificate type's profile** — whatever EKUs a CSR carried were copied onto the certificate (minus only OCSPSigning/timeStamping), so a protocol enrollee could mint itself a code-signing certificate through ACME. Protocol paths now honor only the EKUs the resolved certificate type permits; `anyExtendedKeyUsage` and Smartcard Logon are never honored from a CSR. Operators keep full control through the explicit extra-EKUs option when signing. Contributed by @heidrickla (#243).
- **Sub-CA creation through Sign-CSR is gated and pathLen-clamped** — signing a CSR as `intermediate_ca` only required certificate-signing permissions and copied the CSR's BasicConstraints (pathLen included) unchecked. It now additionally requires `write:cas`, and the sub-CA's pathLen is clamped below the parent's (refused outright under a pathLen-0 parent), per RFC 5280 §4.2.1.9. Contributed by @heidrickla (#243).
- **The private-address guard applies to ACME IP-address orders** — both challenge validators skipped the SSRF check for RFC 8738 `ip` identifiers (it was gated on the identifier being a DNS name), so a deployment that had explicitly disabled `allow_private_ips` still fetched challenges from private and link-local addresses, including cloud metadata endpoints. Deployments using the default (private IPs allowed — the on-prem use case) are unaffected. Contributed by @heidrickla (#244).
- **Missing encryption keys are loud, and can be made fatal** — without a configured database encryption key, integration secrets are protected by a key derived from the machine id (world-readable, routinely captured in backups and images), and without private-key encryption enabled, CA and certificate keys are stored unencrypted — both silently. Both conditions are now logged at startup, and two opt-in environment variables (`UCM_REQUIRE_DB_ENCRYPTION_KEY`, `UCM_REQUIRE_KEY_ENCRYPTION`) refuse to run without explicit keys. **Upgrade note:** startup behavior is unchanged for existing deployments and existing data stays readable; the new startup error on installs that never enabled private-key encryption is informational — enable it under Settings → Security. Contributed by @heidrickla (#245).
- **mTLS, API-key, TSA, CSRF, SSH and OCSP hardening** — issuing an mTLS client certificate now requires `write:user_certificates` (it accepted any authenticated role, and fell back to an arbitrary CA when none was configured); API keys are capped at auth time to their owner's *current* effective permissions, so a key minted by a since-demoted operator no longer keeps its old scopes (**upgrade note:** keys whose owner lost roles or groups lose those scopes immediately — re-mint under an appropriate owner if an integration breaks); the mTLS enrollment endpoints lost their CSRF exemption; SSH certificate extension requests are validated against the CA's configured extension list; OCSP nonces are bounded per RFC 8954; a new opt-in TSA setting can require a dedicated timestamping certificate (the endpoint itself remains public, as RFC 3161 requires); and 2FA recovery codes work again — login compared a truncated prefix, so account recovery by code always failed. Contributed by @heidrickla (#246).
- **Deployment and CI defaults hardened** — the standalone database initializer now flags the seeded admin for a forced password change like every other bootstrap path; the Docker environment example no longer ships a working database password; the development compose file binds the debugger to loopback (the remaining dev ports followed in #255); the Trivy CI action is pinned to a release SHA; OPNsense imports verify TLS by default (**upgrade note:** API callers that relied on the old default must pass `verify_ssl: false` explicitly; the UI checkbox caught up in #255); and EST behind a TLS-terminating reverse proxy requires an explicit verification-success header instead of treating its absence as success (**upgrade note:** proxies must forward `ssl_client_verify`; clients fall back to HTTP Basic authentication otherwise). Contributed by @heidrickla (#248).
- **Every port the development compose file publishes is loopback-bound** — #248's banner claimed the Werkzeug debugger stayed on the developer's machine, but only the debugpy port was bound to 127.0.0.1: the Flask debug server, whose traceback pages disclose source and variable contents to any client with no authentication (and whose PIN-gated console is code execution), was still published on all interfaces. The dev overlay now binds the app, debugger and Mailhog ports to 127.0.0.1 using `ports: !override`, because Compose merges port lists — a plain `127.0.0.1:` prefix would have silently kept the base file's all-interfaces publishes alive. Docker-in-a-VM setups can set `UCM_DEV_BIND` to a VM-internal interface deliberately. **Upgrade note:** `!override` requires Docker Compose ≥ 2.24; on an older release the development stack stops with a YAML parse error naming `!override`, which is the intended failure — it is preferable to silently reverting to an all-interfaces bind. Production compose files are untouched. Contributed by @heidrickla (#255).
- **The Docker environment example's placeholder password is genuinely non-working now** — #248 replaced the example's real default with `REPLACE_WITH_A_STRONG_RANDOM_PASSWORD` and its comment claimed a copied file would fail loudly, but that placeholder is a perfectly valid PostgreSQL password: `cp .env.docker.example .env && docker-compose up -d` stood up a database whose password is public knowledge. The value is now left empty, which the official postgres image refuses to initialize ("Database is uninitialized and superuser password is not specified"), so an unedited copy fails at first startup instead of running with a known password. Contributed by @heidrickla (#255).
- **The OPNsense import form verifies TLS by default** — #248 flipped the backend default, but the form always sends the flag explicitly and its "Ignore SSL certificate (self-signed)" checkbox started checked, so the interactive import — which carries the firewall's API credentials and pulls back private keys — still ran unverified. The checkbox now starts unchecked, a config saved before the option existed no longer re-disables verification on reload, opting out shows the same man-in-the-middle warning as the SSO forms, and the backend logs the opt-out at WARNING (**upgrade note:** the connection test against a stock OPNsense self-signed GUI certificate now fails until you install a trusted certificate or tick the ignore box deliberately — and both import endpoints now say "SSL certificate verification failed" for that case, instead of the generic connection-failure message that pointed at host and port). Contributed by @heidrickla (#255).

### Fixed
- **Admin database reset can no longer wipe the database and then fail to recreate the admin** — the reset endpoint constructed the seeded admin with a keyword the User model does not have (`is_active`; the column is `active`), so the TypeError fired *after* all tables were dropped and surfaced only as a generic "Database reset failed", leaving an empty database with no admin user at all. The standalone `init_db.py` initializer had the same broken constructor and additionally looked up a hard-coded `admin` regardless of `INITIAL_ADMIN_USERNAME`. Both paths now seed the configured admin identity with the same properties as the canonical bootstrap paths — the reset endpoint previously recreated a hard-coded `admin` with no 2FA exemption, so on a deployment with a custom admin name it restored the wrong account, and under global 2FA enforcement it produced an admin nobody could log in as, from a UI button. A guard test keeps every seeding call site aligned with the canonical shape (configured identity, 2FA exemption, the pinned password-hashing helper), not merely with the model's column names. Contributed by @heidrickla (#255).
- **EST no longer logs a false "refusing client cert" warning on every Basic-auth request** — #248's fail-closed proxy check warned whenever the verification header was missing, but that is the normal state for plain Basic-auth enrollments behind the documented nginx proxy (which forwards no `ssl_client_verify` at all), so every successful enrollment logged a refusal of a certificate that was never presented — twice per server-side key generation. The warning now fires only when a certificate actually accompanied the request; the no-certificate path logs at debug and the fail-closed behavior itself is unchanged (and now regression-tested). Contributed by @heidrickla (#255).
