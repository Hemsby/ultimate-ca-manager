...
- **EST enrollment and certificate renewal no longer silently narrow a device's key purposes** — the new EKU cap left EST and scheduled auto-renewal signing under the default TLS-server profile, so a certificate legitimately carrying ipsecIKE, emailProtection or codeSigning came back from renewal without it: the client gets a 200 and a valid certificate, the only trace is a server-side warning, and the VPN/S/MIME/signing service breaks downstream. Renewal now keeps the EKUs the certificate being renewed already carries (renewal at par — OCSPSigning, timeStamping, `anyExtendedKeyUsage` and Smartcard Logon are still never resurrected), and EST signs under a device profile matching the allow-list SCEP already applies (serverAuth, clientAuth, emailProtection, codeSigning, ipsecIKE). Contributed by @heidrickla (#254).
