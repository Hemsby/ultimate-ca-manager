...
This patch implements interrelated features for the Ultimate Certificate Manager (UCM):

1. **Deletion guard** — prevents deletion of valid (non-revoked, non-expired) certificates without prior revocation.
2. **Persistent revocation records** — a new `revoked_serials` table stores revocation metadata independently of the `certificates` table, so revocation data survives certificate deletion and continues to appear in CRLs and OCSP responses until the original certificate expires.
3. **Renewal rework** — a shared `renew_certificate_in_place` function (`services/cert/renewal.py`) is the single implementation used by all three renewal callers (manual, bulk, auto). It records the old serial in `revoked_serials` (reason: `superseded`) with `certificate_id` preserved, then updates the existing certificate row in-place with a new serial number, key, and validity. The certificate's `id`, `refid`, and `created_at` are preserved. `renewed_at` (DateTime) and `renewed_times` (Integer) columns track the last renewal timestamp and count.
4. **Auto-renewal rewrite** — `AutoRenewalService` now delegates to the same shared renewal function with `rekey=False` (protocol-enrolled certs keep their client-side key). The old create-new-row + archive-old-row pattern is eliminated, giving unified renewal semantics across manual, bulk, and automatic renewal.
5. **Private key encryption at rest** — renewed private keys are encrypted with `encrypt_private_key` exactly as at issuance, preventing a silent downgrade to plaintext on deployments with `master.key` configured.
6. **Revoked certificate renewal guard** — `renew_certificate_in_place` rejects revoked certificates with HTTP 409, preventing a renewal from silently clearing revocation state.
7. **Atomic revocation persistence** — `revoke_certificate` now flushes the certificate revocation and the `RevokedSerial` record in a single transaction, so a worker can never observe a revoked cert without its persistent record.
8. **Type-safe JSON config parsing** — `auto_renewal_service.py` validates that `auto_renewal_sources` and `auto_renewal_notify_emails` parse to lists, falling back to defaults instead of silently iterating characters.
9. **Atomic unhold** — `unhold_certificate` now clears the cert revocation and deletes the `RevokedSerial` record in a single transaction, so a worker can never observe an unheld cert whose serial is still on the CRL.



### Security
- Certificates: a valid, non-revoked X.509 certificate can no longer be deleted — deletion removed the record while the certificate stayed trusted until expiry, with no CRL/OCSP trace; revoke it first so relying parties see the change (single delete returns 409, bulk delete reports the item as failed). Expired or revoked certificates, and CSR-only records, are unaffected. Note: after a renewal, the superseded certificate must now be revoked before it can be deleted manually (#296, contributed by @gb-123-git & PMGA Tech LLP)
...
- SSH certificates: a valid, non-revoked certificate can no longer be deleted — deletion silently dropped the record while the certificate stayed trusted by servers until expiry; revoke it first (the API returns 409 otherwise) (#292, contributed by @gb-123-git & PMGA Tech LLP)
...
- ACME local domains: bare private TLDs (e.g. `local`, `internal`) can now be registered, covering all their subdomains through the existing parent matching (#290, contributed by @gb-123-git & PMGA Tech LLP)
...
- `app.config['DATA_DIR']` is now set, so the session cleanup task no longer silently falls back to the default data directory on custom layouts (#290, contributed by @gb-123-git & PMGA Tech LLP)
...
- **SSH certificate issuance UI** — the certificate type selector now auto-syncs to the selected SSH CA (contributed by @gb-123-git & PMGA Tech LLP).

### Security
- **Community audit round-up** (contributed by @gb-123-git & PMGA Tech LLP #263): path traversal in the update-download helper (unsanitized `package_name` reached a file path), path verification on backup download/delete (filenames are now resolved and confined to the backup directory), SQL-injection hardening of the PostgreSQL sequence-reset helper (compiler-rendered identifiers and bound names, per-column savepoints), missing ownership checks on ACME-proxy resources — account status enforced at new-order, DNS automation only triggers for the order's owner, challenge-to-order attribution fails closed when the upstream CA omits the `Link rel="up"` header (legacy orders recorded before ownership tracking intentionally keep read access) — IDOR fixes on user-certificate revoke/delete, SSO provider secrets no longer returned to non-admins requesting `include_secrets`, CA export password removed from the URL query string (out of access logs), LDAP bind passwords encrypted at rest, and security-sensitive settings (session/lockout/HSTS/public-URL/password-policy keys) now require `admin:settings`, with the settings UI locking those fields for operators instead of failing the whole card save.
- **WebSocket security overhaul** (contributed by @gb-123-git & PMGA Tech LLP #263): socket handshakes previously skipped authentication entirely, so any connection received every certificate/CA broadcast, and authenticated clients could subscribe to arbitrary rooms. Connections now authenticate through the Flask session (or an API key scoped to its owner's current permissions, with mandatory periodic re-authentication) and fail closed on error, events are emitted to server-managed permission-scoped rooms only, clients cannot leave mandatory rooms or join privileged ones without the underlying permissions, subscriptions are rate-limited, sessions are periodically re-validated against the auth manager, and API-key authentication in the URL query string is deliberately unsupported.
...
- **SSH CA download/copy public-key buttons** and the missing `openssh-client` package in the Docker image for KRL generation (contributed by @gb-123-git & PMGA Tech LLP).
