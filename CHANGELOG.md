...
- **CSR-supplied Extended Key Usages are capped to the certificate type's profile** — whatever EKUs a CSR carried were copied onto the certificate (minus only OCSPSigning/timeStamping), so a protocol enrollee could mint itself a code-signing certificate through ACME. Protocol paths now honor only the EKUs the resolved certificate type permits; `anyExtendedKeyUsage` and Smartcard Logon are never honored from a protocol CSR. Operators keep full control through the explicit extra-EKUs option when signing. A CSR whose *every* EKU is refused receives the certificate type's default profile instead of a certificate with no EKU extension at all — which validators treat as valid for every purpose, i.e. strictly weaker than the cap — and a type with no profile to fall back to (e.g. `custom`) now refuses such a CSR outright. **Upgrade note:** a CSR that omits EKUs entirely now receives the type's default profile on every path, including the admin Sign-CSR endpoint — previously `code_signing` and `email_cert` issued such CSRs with no EKU extension at all, i.e. unrestricted (a no-EKU CSR signed under `custom` still does). A certificate that relied on that — e.g. an S/MIME certificate also used for TLS client authentication — should request its purposes in the CSR or via extra EKUs: CSR-supplied EKUs on the admin path remain uncapped, and `extra_ekus` merges additional purposes on any path. Contributed by @heidrickla (#243, #254).
...
- **EST enrollment and certificate renewal no longer silently narrow a device's key purposes** — the new EKU cap left EST and scheduled auto-renewal signing under the default TLS-server profile, so a certificate legitimately carrying ipsecIKE, emailProtection or codeSigning came back from renewal without it: the client gets a 200 and a valid certificate, the only trace is a server-side warning, and the VPN/S/MIME/signing service breaks downstream. Renewal now keeps the EKUs the certificate being renewed already carries, whether or not the renewal CSR requests them — scheduled auto-renewal re-signs the stored enrollment CSR, which typically carries no EKU at all (renewal at par — OCSPSigning, timeStamping, `anyExtendedKeyUsage` and Smartcard Logon are still never resurrected), and EST signs under a device profile matching the allow-list SCEP already applies (serverAuth, clientAuth, emailProtection, codeSigning, ipsecIKE), still carrying the TLS KeyUsage pair EST has always issued for CSRs that specify none. Contributed by @heidrickla (#254).
- **The sub-CA pathLen rules apply on every route that mints a CA** — the new clamp read BasicConstraints from the CSR, but UCM's own CSR generator emits none, so signing such a CSR as `intermediate_ca` under a pathLen-0 parent still succeeded on the default API path; `POST /api/v2/cas` likewise accepted any `pathLength` without consulting the parent, and omitting the field under a constrained parent minted an *unlimited* child. Both routes now enforce the same RFC 5280 §4.2.1.9 rule: refused under a pathLen-0 parent, capped below the parent's limit otherwise, and the stored CA record always matches the certificate it describes. Contributed by @heidrickla (#254).
