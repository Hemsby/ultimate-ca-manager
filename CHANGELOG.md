...
### Fixed
- Dashboard: the SCEP service tile now reflects the SCEP configuration instead of always reporting the endpoint as available. It reads Disabled when SCEP is turned off, Enabled with no CA assigned when neither the global CA nor an enabled SCEP profile can serve an enrollment, and Configured (with the enabled profile count) otherwise (#328, reported by @JoseGoncalves)
