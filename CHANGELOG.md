...
- **ACME challenge validation never fetches from a cloud metadata endpoint** — the private-address guard only runs where `allow_private_ips` is turned off, so a default installation (private IPs allowed, since on-prem issuance is the point) still issued a challenge request to `169.254.169.254` for an order naming it — the one address that leaks cloud credentials. Both challenge validators now refuse metadata endpoints regardless of the setting, matching what the rest of UCM already does for admin-supplied URLs. The deny-list also gained the AWS ECS/EKS **task-credentials** endpoint (`169.254.170.2`), which was absent because it is not the *instance* metadata service — but it hands out live task-role IAM credentials just the same, so it was the one credential endpoint every guard in UCM would have fetched from. Private, LAN and loopback targets that resolve keep working wherever the setting allows them — including hostnames that merely contain words like "metadata" or "loopback". Contributed by @heidrickla (#256).
- **An ACME `dns` identifier must actually be a domain name** — its value was never validated, so a client could order `169.254.169.254:80` or `evil@10.0.0.5`: unresolvable as a *host name*, which is precisely why the private-address guard let it past, but the HTTP client then re-read it as a URL authority, dropped the port or user info and connected to the address behind it — the same private-network request the guard exists to prevent, even with `allow_private_ips` explicitly disabled. Such values are now rejected as malformed when the order is created, and where the private-address guard runs the challenge request is pinned to the addresses it validated — the whole resolved set, since the guard refuses the host if *any* address is forbidden — so a second DNS answer cannot redirect the fetch and multi-address failover still works. Contributed by @heidrickla (#256).
- **The SSRF guards fail closed on an unresolvable host** — both the private-address guard and the shared cloud-metadata guard treated "cannot resolve" as "cannot reach anything" and let the value through, but the HTTP client re-parses such values as a URL authority and resolves them on its own, and a resolver that answers the fetch while failing the check would have switched the guard off entirely. Both guards now refuse a host they cannot resolve. Guard verdicts are also decided from the parsed address itself, never from matching exception text — the old message-matching refused any hostname merely *containing* "metadata", "loopback", "unspecified", "private" or "reserved" (e.g. `metadata-db.corp.example.com`), terminally invalidating ACME orders for such names in every configuration. The DNS-pinned `safe_request_*` helpers (webhooks, SSO, OIDC, ACME client and proxy) also pin to every address they validated instead of only the first, so a multi-A or dual-stack upstream keeps its connection failover. **Upgrade note:** URLs and ACME identifiers whose hostname does not resolve from UCM's viewpoint are now rejected at validation time (admin URL fields, webhook and OPNsense targets, ACME challenge validation) — previously they were accepted and failed only on connect. Contributed by @heidrickla (#256).
...
- **Outbound calls to a multi-address host keep their failover** — the DNS-rebinding protection on admin-driven outbound HTTP (webhook delivery and tests, SSO/SAML metadata and OIDC discovery, ACME client and proxy upstream calls) validated every address a hostname resolved to but then pinned the connection to the first one, so a host published behind several A records or dual-stack was reachable at exactly one of them and a single dead address broke the call. The connection is now pinned to the whole validated set and tried in resolver order, as an unpinned client would. Security is unchanged: the guard still refuses the host outright if *any* resolved address is a cloud metadata endpoint or loopback, so every pinned address has already passed the same check. Contributed by @heidrickla (#256).
