...
- **The ACME Terms-of-Service preview no longer interpolates HTML** — the preview was built by a hand-rolled regex sanitizer whose output went to `dangerouslySetInnerHTML`; it did not escape double quotes and never validated link schemes, so admin-supplied ToS text was stored XSS against any operator viewing the ACME settings page (defence in depth — the CSP already blocked inline script execution on modern browsers). The preview now renders through ReactMarkdown with an http/https/mailto scheme allowlist; bare-URL autolinking, single-newline line breaks and list styling are preserved. Contributed by @heidrickla (#247).
