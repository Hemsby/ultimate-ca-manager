    ...
    sections: [
      ...
      },
      {
        ...
        items: [
          ...
          { label: 'アカウントのメールアドレス (v2.218)', text: 'ローカルアカウントの連絡先メールアドレスは詳細パネルから編集できます。RFC 8555 では連絡先は ACME クライアントに属するため、クライアントの次回のアカウント更新で値が上書きされる可能性があります' },
          { label: '履歴', text: 'すべてのACME証明書発行オーダーの追跡' },
                  { label: 'ローカル注文 (v2.217)', text: 'Local orders タブは内蔵サーバーの注文をステータスフィルタ、個別削除、パージ付きで一覧します。期限切れの未発行注文は 6 時間ごとに自動パージされ、プロファイルなしの注文には既定の署名ダイジェストが適用されます。v2.218 以降、パージは検証済みの認可に残っている保留中の兄弟チャレンジも削除します' },
                  { label: 'プロファイルとテンプレート (v2.221)', text: '証明書プロファイル（draft-ietf-acme-profiles）はディレクトリで公開され、クライアントが注文ごとに選択します。各プロファイルは有効期間とダイジェストを定め、証明書テンプレートを紐付けると、そのKU/EKUが発行される証明書を決定します' },
        ]
      },
      {
        title: 'ACMEプロキシ',
        items: [
          ...
          { label: '置き換えられた証明書の削除', text: 'オプトインのトグル：プロキシオーダーの確定時に、まったく同じドメインセットに対して以前プロキシオーダーでインポートされた証明書を削除します。失効した証明書は常に保持され、プロキシ以外の証明書には一切触れません。デフォルトは無効' },
                  { label: 'クライアント事前チェック (v2.217)', text: 'プロキシは各クライアントのアカウント鍵で計算した dns-01 TXT 値も公開します。lego/Traefik/Caddy の伝播事前チェックは無効化せずに通過し、再試行された new-order はアカウントの保留中の注文を返します' },
        ]
      },
      {
